...
// make a valid one. Set codes are lowercased to match catalog card keys; the
// finish/language dropdowns already offer only valid values, but the input
// is still validated here rather than trusted, like every other field.
...
export function toAddCardsRows(list: StagedEntry[]): Array<{
  setCode: string;
  collectorNumber: string;
  finish: Finish;
  language: Language;
