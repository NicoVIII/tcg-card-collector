...
- **Page plumbing that repeats becomes a helper**: the seed-once `createEffect` pattern still awaits its extraction. The `onError: mapError(...).message` pattern has been extracted to `lib/mutation_error.ts` (`createMutationError`, scoped so more than one form on a page can each show its own error) and adopted in `placement_page.tsx` and `insights_page.tsx`; `inventory_page.tsx`'s four copies are the next planned adopter. `catalog_page.tsx`, `add_cards_panel.tsx`, and `collection_import_page.tsx` wrap the message in richer feedback objects and are left alone for now — a future occurrence there is a separate call, not an oversight.
