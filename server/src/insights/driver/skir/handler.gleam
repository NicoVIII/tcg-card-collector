...
fn handle_mark_target_set(
  ...
  fn(req: insights_commands.MarkTargetSetRequest, _, ctx) {
    ...
    |> insights_skir_codec.map_mark_target_set_result
    |> helpers.respond
  }
}
...
fn handle_unmark_target_set(
  ...
  fn(req: insights_commands.UnmarkTargetSetRequest, _, ctx) {
    ...
    |> insights_skir_codec.map_unmark_target_set_result
    |> helpers.respond
  }
}
...
fn handle_get_set_completion(
  ...
  fn(_: insights_queries.GetSetCompletionRequest, _, ctx) {
    set_completion_handler.execute(
      set_completion_handler.SetCompletionQuery,
      get_dependencies(ctx).set_completion_ports,
    )
    |> helpers.map_query(insights_skir_codec.map_set_completion_list)
    |> helpers.respond
  }
}
