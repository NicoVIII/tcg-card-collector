...
pub type MarkCardsPlacedCommand {
  MarkCardsPlacedCommand(placements: List(RawPlacement))
}

pub fn execute(
  command: MarkCardsPlacedCommand,
  port: ports.MarkCardsPlacedPort,
) -> command_result.CommandResult(ports.MarkCardsPlacedError) {
...
fn guard_non_empty(
  ...
  continue: fn() -> command_result.CommandResult(ports.MarkCardsPlacedError),
) -> command_result.CommandResult(ports.MarkCardsPlacedError) {
  case placements {
