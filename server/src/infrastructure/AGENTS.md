...
**Catalog import flow.** `catalog_store.import_cards` orchestrates a validate/partition pass: jq projects Scryfall bulk JSON to compact NDJSON (one object per card, 6 fields), Gleam reads the file, constructs and validates each row via `card_printing.from_raw`, skips invalid rows with a per-row warning, then writes the valid rows back as CSV for the existing fast sqlite `.import` + atomic full-replace. `simplifile` is the sanctioned library for in-process file I/O.
